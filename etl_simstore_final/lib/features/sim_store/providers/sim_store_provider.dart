...
final wantedNumbersProvider = StateProvider<String>((ref) => '');
final unwantedNumbersProvider = StateProvider<String>((ref) => '');
...
final filteredSimCardsProvider = Provider<AsyncValue<List<SimCard>>>((ref) {
  ...
  final wantedNumbers = ref.watch(wantedNumbersProvider);
  final unwantedNumbers = ref.watch(unwantedNumbersProvider);
    ...
    data: (simCards) {
      var filtered = simCards.where((sim) {
        ...
        if (wantedNumbers.isNotEmpty) {
          final wantedList = wantedNumbers
              .split(',')
              .map((e) => e.trim())
          ...
          if (wantedList.isNotEmpty) {
            ...
            for (String wanted in wantedList) {
              if (sim.phoneNumber.contains(wanted)) {
        ...
        if (unwantedNumbers.isNotEmpty) {
          final unwantedList = unwantedNumbers
              .split(',')
              .map((e) => e.trim())
              .where((e) => e.isNotEmpty)
              .toList();

          for (String unwanted in unwantedList) {
            if (sim.phoneNumber.contains(unwanted)) {
              return false;
            }
          }
        }
